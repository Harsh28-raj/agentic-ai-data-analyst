...
<p>
An autonomous <b>LangGraph</b> agent that reasons over your CSV data the way a human analyst would —
understanding intent, writing SQL, running it, visualizing results, and explaining every step.
</p>
...
**DataMind AI** is an enterprise-grade **agentic AI platform** that turns raw CSV datasets into a conversational analytics experience. Instead of writing SQL, wrangling pivot tables, or waiting on a BI team, users simply **ask a question in plain English** — and an autonomous LangGraph agent handles the rest: understanding intent, generating and executing SQL against DuckDB, analyzing results, producing interactive visualizations, flagging anomalies, and explaining exactly how it got there.

This is **not a thin wrapper around an LLM API call**. It's a full agentic pipeline with tool orchestration, session-aware memory, structured error handling, and a production-style architecture built to scale from a single CSV to multi-file, multi-session workloads.

> [!TIP]
> Think of it as **"a data analyst who never sleeps"** — self-hosted, transparent about its reasoning, and built to show its work: the SQL, the chart, and the *why*, every single time.
...
| Environment | Link |
|:---|:---|
| 🖥️ **Frontend (Streamlit App)** | [datamind-ai-frontend.onrender.com](https://datamind-ai-frontend.onrender.com) |
| ⚙️ **Backend API (Swagger UI)** | [datamind-ai-backend.onrender.com/docs](https://datamind-ai-backend.onrender.com/docs) |
...
| 🧠 **Full reasoning transparency** | Every response includes the generated SQL, the chosen tool, and a plain-English explanation |
...
| 🗂️ **Clean architecture** | Clear separation of `agent/`, `api/`, `core/`, and `frontend/` — each with a single responsibility |
...
This tool-orchestration behavior — reason → act → observe → repeat — is the **ReAct pattern**, and it's what separates an "agent" from a chatbot with API access.
...
- **Persistent state & checkpointing** — conversation memory survives across turns within a session
...
- Pandas fallback for complex transformations
...
- Trend detection across time series
...
| Statistical **+** ML anomaly detection | ❌ | ⚠️ Add-on | ✅ Built-in |
...
> [!NOTE]
> Every arrow into **Groq Cloud** represents a distinct reasoning step — intent classification, SQL generation, and insight synthesis are handled as separate, inspectable LLM calls rather than one opaque prompt.
...
**The loop above is Reason → Act → Observe → Repeat** — the agent doesn't just answer, it decides *how* to answer, checks its own results, and only then responds.
...
<details open>
<summary><b>1️⃣ Intent Understanding</b></summary>
...
<details>
<summary><b>2️⃣ SQL Generation Flow</b></summary>
...
User Query + Schema Context
        │
        ▼
  LLM drafts SQL (DuckDB dialect)
        │
        ▼
  Syntax validation
        │
        ▼
  Execute against in-memory DuckDB table
        │
        ▼
  Results returned to agent state
```

The generated SQL is **never hidden** — it's surfaced in the response so users (and reviewers) can verify exactly what was queried.

</details>

<details>
<summary><b>3️⃣ Anomaly Detection Flow</b></summary>
...
<details>
<summary><b>4️⃣ Insight & Chart Synthesis</b></summary>

<br/>

Once query results are available, the agent selects an appropriate chart type based on the data shape (categorical → bar, time-series → line, distribution → scatter/pie), renders it via Plotly, and generates a concise business-language summary alongside the raw numbers.
...
|---|---|---|
| **Agent Orchestration** | LangGraph | Stateful, branching agent workflows with memory |
...
│   └── tools/                     # SQL, chart, anomaly, insight tools
│
...
├── 🖼️ images/                    # Screenshots, banner, GIF assets
├── 📜 logs/                      # Runtime logs
│
├── 🐳 Dockerfile.backend
├── 🐳 Dockerfile.frontend
...
|---|---|---|
| 🐍 Python | 3.11+ | Required for backend & frontend |
...
# 6️⃣ Run the frontend (in a separate terminal)
streamlit run frontend/app.py
```

| Service | URL |
|---|---|
| ⚙️ Backend (Swagger UI) | http://localhost:8000/docs |
...
> [!WARNING]
> Never commit a real `.env` file. Only `.env.example` should be tracked in version control — it's already covered by `.gitignore`.
...
| Service | Port | URL |
|---|---|---|
| ⚙️ Backend (FastAPI) | `8000` | http://localhost:8000/docs |
| 🖥️ Frontend (Streamlit) | `8501` | http://localhost:8501 |
...
| Service | URL |
|---|---|
| 🖥️ Frontend | [datamind-ai-frontend.onrender.com](https://datamind-ai-frontend.onrender.com) |
| ⚙️ Backend Swagger Docs | [datamind-ai-backend.onrender.com/docs](https://datamind-ai-backend.onrender.com/docs) |
| 📄 OpenAPI Spec | [datamind-ai-backend.onrender.com/openapi.json](https://datamind-ai-backend.onrender.com/openapi.json) |

<p align="center">
<img src="images/render-dashboard.png" alt="Render Deployment Dashboard" width="80%"/>
<br/>
<i>Render service dashboard — placeholder</i>
</p>
...
<details>
<summary><b>☁️ Streamlit Community Cloud (frontend only)</b></summary>
...
Interactive, auto-generated docs are live at **[datamind-ai-backend.onrender.com/docs](https://datamind-ai-backend.onrender.com/docs)** (Swagger UI) and the raw schema at **[/openapi.json](https://datamind-ai-backend.onrender.com/openapi.json)**.
...
<img src="images/demo.gif" alt="DataMind AI Demo" width="90%"/>

<i>Placeholder — replace with a recorded walkthrough GIF/video before submission</i>
...
| 🧱 **Horizontal scalability** | Stateless FastAPI layer + session-scoped DuckDB contexts allow multiple backend instances behind a load balancer |
| 🗃️ **Session isolation** | Each upload session gets its own in-memory DuckDB context — no cross-session contention |
...
## 🗂️ Session Management & Error Handling
...
- [LangChain](https://www.langchain.com/) & [LangGraph](https://www.langchain.com/langgraph) — the agentic orchestration framework
